...
export function initIntroAnimation(innerGroupCount, outerGroupCount, callback) {
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            const moveUpDelay = 8; // Delay before moving up in seconds
            setTimeout(() => {
        ...
        setTimeout(() => {
            const blueDots = smallDots.slice(0, blueDotsCount);
            const redDots = smallDots.slice(blueDotsCount, innerGroupCount);

            const totalInnerDots = blueDotsCount + redDotsCount;
            const angleIncrement = (Math.PI * 2) / totalInnerDots;

            blueDots.forEach((dot, index) => {
                const angle = angleIncrement * index;
                const radius = 30 + Math.random() * 65; // Random radius for variation
                const targetX = centerX + radius * Math.cos(angle); // Maintain circular shape
                const targetY = centerY + radius * Math.sin(angle); // Maintain circular shape
            ...
            redDots.forEach((dot, index) => {
                const angle = angleIncrement * (index + blueDotsCount);
                const radius = 30 + Math.random() * 65; // Random radius for variation
                const targetX = centerX + radius * Math.cos(angle); // Maintain circular shape
                const targetY = centerY + radius * Math.sin(angle); // Maintain circular shape
        ...
        }, 7000); // Adjust the delay to match the timing of the color change
    });
}