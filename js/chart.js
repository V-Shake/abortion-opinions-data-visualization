...
export function renderChart(
	...
	const totalCountDataset1 = radarDataList[0].reduce(
		(sum, d) => sum + d.value,
		0
	); // Total for dataset 1
	const totalCountDataset0 = radarDataList[1].reduce(
		(sum, d) => sum + d.value,
		0
	); // Total for dataset 0
	...
	subcategories.forEach((subcat, i) => {
		...
		svg.append("text")
			...
			.attr(
				"transform",
				`rotate(${rotationAngle}, ${labelCoord.x}, ${labelCoord.y})`
			) // Apply rotation
			.text(subcat)
			...
			.on("mouseover", () => {
				...
				const percentage1 =
					totalCount > 0
						? ((valueDataset1 / totalCount) * 100).toFixed(0)
						: 0; // Calculate percentage for dataset1
				const percentage0 =
					totalCount > 0
						? ((valueDataset0 / totalCount) * 100).toFixed(0)
						: 0; // Calculate percentage for dataset0
				...
				if (
					...
					tooltip.html(
						`${subcat} year olds:<br>${percentage1}% support, ${percentage0}% oppose`
					); // Tooltip for age categories
				} else {
					tooltip.html(
						`${subcat}s:<br>${percentage1}% support, ${percentage0}% oppose`
					); // Simpler tooltip for other categories
				}
			})
			.on("mousemove", (event) => {
				...
				if (
					...
					tooltip
						.style("top", event.pageY + 10 + "px") // Keep the vertical position the same
						.style(
							"left",
							event.pageX -
					...
					tooltip
						.style("top", event.pageY + 10 + "px") // Offset tooltip position from mouse Y
	...
	Object.keys(mainCategoryPositions).forEach((category) => {
		...
		const averageIndex =
			subcatIndices.reduce((a, b) => a + b) / subcatIndices.length; // Calculate average index
		...
		if (category === "Gender") {
			...
			averageIndex >= 0 &&
			averageIndex < subcategories.length / 2
		) {
	...
	if (colorMode == 0) {
		...
		coordinatesWithAreas.forEach((item) => {
			...
			const path = svg
				...
				.attrTween("d", function () {
					return function (t) {
						const interpolatedCoordinates = item.coordinates.map(
							(coord) => ({
		...
		coordinatesWithAreas.forEach((item) => {
			const color =
				opinion === "support" ? colors.dataset1 : colors.dataset0;
			const glowColor =
				opinion === "support"
					? colors.glowDataset1
					: colors.glowDataset0;
			const gradientId =
				opinion === "support" ? "gradientDataset1" : "gradientDataset0";
	...
	const purpleArcForEducation = d3
		.arc()
		.innerRadius(innerRadius) // Use the new inner radius
		.outerRadius(outerRadius) // Use the new outer radius
	...
	const genderLabelCoord = angleToCoord(
		(angleFemale + angleMale) / 2,
		maxValue + 4
	); // Midpoint for Gender arc
	svg.append("text")
	...
	const partyLabelCoord = angleToCoord(
		(angleRepublican + angleOther) / 2,
		maxValue + 4
	); // Midpoint for Party arc
	svg.append("text")
	...
	const educationLabelCoord = angleToCoord(
		(angle0_9 + angle16) / 2,
		maxValue + 4
	); // Midpoint for Party arc
	svg.append("text")
	...
	const ageLabelCoord = angleToCoord(
		(angle18_30 + angle61_89) / 2,
		maxValue + 4
	); // Midpoint for Party arc
	svg.append("text")
